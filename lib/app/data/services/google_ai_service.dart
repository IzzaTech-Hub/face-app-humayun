...
class GeminiImageService {
  ...
  Future<GeminiImageResponse> generateGeminiImage({
    ...
    try {
      ...
      print('Making API request to Gemini for prompt: $prompt');
      ...
      print('Decoded response structure: ${decoded.keys}');
      ...
      print('Number of parts in response: ${parts.length}');

      // Look for image data in any part
      for (int i = 0; i < parts.length; i++) {
        final part = parts[i];
        print('Part $i keys: ${part.keys}');

        if (part['inlineData'] != null && part['inlineData']['data'] != null) {
          final base64Image = part['inlineData']['data'] as String;
          print('Found image data, base64 length: ${base64Image.length}');

          try {
            imagebytes = base64Decode(base64Image);
            print('Successfully decoded image bytes: ${imagebytes.length}');
            ...
            if (imagebytes.length > 1000) {
              ...
              print('Image bytes too small: ${imagebytes.length}');
            }
          } catch (e) {
      ...
      print('Exception in generateGeminiImage: $e');
      return GeminiImageResponse(
        success: false,
